...
**版本**：Phase K.2（2026-04-21）· 196 条规则 / 73 条 ACTIVE · 230 个测试通过
...
- **不是完整法规库**。196 条 seed 规则（经 human-review rounds 1–3 之后已有 73 条 ACTIVE）覆盖主要 EU + DE / UK / ES / FR 框架；剩余 123 条非 ACTIVE 规则在卡片上都会注明"为什么还未 promoted"的具体原因。
- **不是万能市场工具**。当前覆盖 DE（8 ACTIVE）+ UK（11 ACTIVE）+ ES（7 ACTIVE）+ FR（5 ACTIVE，部分覆盖）；NL 仅 seed（0 ACTIVE）；其他 22 个 EU 国延后；非欧盟市场（CN/US/JP/TR）不支持。
...
- **Countries at risk** — 对目标市场哪些有风险。经 K.2 之后，**DE**（8 ACTIVE）+ **UK**（11 ACTIVE）不出现在 at-risk 列表；**ES**（7 ACTIVE / 7 pending）和 **FR**（5 ACTIVE / 7 pending）以"partial"形式列出并标明剩余数量；**NL**（0 ACTIVE / 5 SEED_UNVERIFIED）诚实显示"seed-only: 5 条已撰写、0 条已验证"。每条 at-risk 都给出原因。
...
**常见错误**：
- **勾选 NL 后看到大量 NL 规则显示 UNKNOWN** —— 这**不是 bug**。NL overlay 目前是 seed-only（0 ACTIVE / 5 SEED_UNVERIFIED），Phase K+ 批次才升 ACTIVE。**FR（5 ACTIVE / 7 pending）和 ES（7 ACTIVE / 7 pending）已部分覆盖**，剩余 pending 会在规则卡片上标注"why indicative only"。ScopeBanner 4 层网格会显式告知。
...
- 第一次上手演示
- 测试工具行为时的标准配置
- 回归基线（pilot 的评估结果已被 230 tests 中的 `pilot-acceptance.test.ts` 固化）
...
显示 196 条规则按 lifecycle 分布（K.2 之后最新）：
...
- 🟡 **ES (7 ACTIVE + 7 indicative/DRAFT/PLACEHOLDER)** — 部分覆盖，每条 indicative 卡片标注"why pending"
...
1. **Production-grade** — DE（8 条 ACTIVE）+ UK（11 条 ACTIVE）+ EU 横向（~45 条 ACTIVE）。可以信赖。
2. **Partial / Indicative** — ES（7 ACTIVE / 7 pending）+ FR（5 ACTIVE / 7 pending）。配合每条卡片"why pending"使用。
3. **Seed-only / Pending** — NL（5 条 SEED_UNVERIFIED，0 条 ACTIVE）。尚未验证。
...
**Phase K+ 会填充 NL ACTIVE 内容 + 完成 ES/FR 剩余验证。**
