...
**版本**：Phase 12 Path B（2026-04-19）
**适用 pilot**：MY2027 BEV × DE
**语言**：中文（English version 规划 Phase 2）
...
- **不是完整法规库**。137 条 seed 规则覆盖主要框架；UNECE Annex II 等空档已显式标注为 placeholder。
- **不是万能市场工具**。当前 DE demo；FR/NL 是 placeholder；其他 EU 国家延后；非欧盟市场不支持。
...
- **Countries at risk** — 对 DE / FR / NL 三个目标市场哪些有风险（FR 和 NL 会显示 "pending overlay — all rules placeholder (Phase 13+)"，这是**诚实**的表达，不是工具在假装覆盖）

### 1.3 切到 Plan tab 看本月任务

点顶部导航 **Plan** 切换。左边是 **Timeline**，按 SOP 日期锚定分段：
...
- ✓ **VERIFIED** — 源已验证，可以信赖
- ⚠ **INDICATIVE** — 已撰写但源未验证，仅供参考
- ○ **PENDING AUTHORING** — 占位，未写
...
**常见错误**：
- **勾选 FR / NL 后看到所有 FR / NL 规则都显示 UNKNOWN** —— 这**不是 bug**。FR 和 NL 的 overlay 是 placeholder，Phase 13+ 才填充。ScopeBanner 会显式告知。
- **勾选非 EU 国家（CN、US、JP 等）** —— 这些国家根本不在下拉选项里。工具明确只服务 EU + UK + EEA + CH 之内。
- **只勾 FR 不勾 DE** —— FR overlay 现在是 placeholder，意味着 Status tab 会显示 "Countries at risk"。可以测试但不是有效 demo 配置。

**影响哪些规则触发**：
- `DE` 触发 REG-MS-DE-001..005（5 条 ACTIVE 国别规则）
- `FR` / `NL` 目前触发 placeholder 规则（全部返回 UNKNOWN，直到 Phase 13）
...
- 第一次上手演示
- 测试工具行为时的标准配置
- 回归基线（pilot 的评估结果已被 204 tests 中的 `pilot-acceptance.test.ts` 固化）
...
- `DE` ✓（有 5 条 ACTIVE overlay）—— 不出现在 at-risk 列表
- `FR` ⚠ pending overlay — **"all rules placeholder (Phase 13+)"**
- `NL` ⚠ 同上
- `UK` ⚠ out of scope — "non-EU market, UK AV Act listed but not fully integrated"
...
Rules tab 的规则按**可信度**分 4 段：

| 分段 | 图标 | 含义 | 默认状态 |
|---|---|---|---|
| **Verified** | ✓ | ACTIVE + source 已验证，可以信 | 展开 |
| **Indicative** | ⚠ | SEED_UNVERIFIED / DRAFT / SHADOW，已撰写但源未验证 | 展开 |
| **Pending authoring** | ○ | PLACEHOLDER，未撰写 | 折叠（显示数量） |
| **Needs your input** | — | 你的 config 缺字段导致 UNKNOWN | 展开（仅非空时） |

**每段 header** 显示规则数 + 说明 hint，例如：
- ✓ VERIFIED (17) — "You can rely on these"
- ⚠ INDICATIVE (5) — "Review before trusting"
- ○ PENDING AUTHORING (40) — "Not yet written up"
...
|---|---|---|
| `not_authored` | "Not authored yet" | 等 Phase 13+ 撰写，或在 content/authoring.csv 里贡献 |
...
### 8.4 ScopeBanner 说 "FR / NL overlay pending" 是什么意思

工具**暂时没有**填充 FR 和 NL 的国别规则。相应的 `REG-MS-FR-*` 和 `REG-MS-NL-*` 都是 PLACEHOLDER。

**你勾选 FR / NL 作为 target country，工具不会假装覆盖** —— 它会：
1. 在 Status tab 的 "Countries at risk" 明确列出
2. 在 ScopeBanner 顶部常驻提示
3. 在 Coverage tab 的 Member-state chips 显示灰色

**Phase 13+ 才会填充 FR / NL 内容**。
...
### B.4 Member-state overlay status · 成员国 overlay 状态（Coverage tab chips）

| 状态 | 文案 |
|---|---|
| 🟢 **Operational** | "Operational guidance available" —— DE 已有 5 条 ACTIVE overlay |
| 🟠 **Placeholder** | "National overlay not yet authored (Phase 13+)" —— FR / NL |
| 🟡 **Basic** | "Basic overlay only — verify locally" —— 骨架 overlay，未 review |
| 🔵 **Non-EU** | "Non-EU market — post-Brexit rules apply" —— UK 等 |
...
| 11 | **27 国 content 扩展** | 当前只 DE。FR / NL placeholder；其他 24 国不做。 |
